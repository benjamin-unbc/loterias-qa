...
class ClientDetailsModal extends Component
{
    ...
    protected function getAnteriorForDate(string $date, int $userId): float
    {
        ...
        if ($selectedDate->isMonday() && !isset($this->anteriorCache[$cacheKey])) {
            ...
            // Calcular UD Deja temporal del sábado (sin comisión)
            $udDejaTemp = $saturdayTotalGanaPase + $fridayAnteri;
        ...
        if (isset($this->anteriorCache[$cacheKey]) && $this->anteriorCache[$cacheKey] !== null) {
            ...
            if ($previousDate->isSaturday() && $weeklyCommissionPercentage > 0) {
                ...
                // Calcular UD Deja temporal del sábado (sin comisión)
                $udDejaTemp = $prevTotalGanaPase + $prevPrevAnteri;
     ...
     * Calcula directamente el UD DEJA sin llamar a computeClientLiquidationDataForDate para evitar recursión
     */
    protected function getUdDejaForDate(string $date, int $userId, int $depth = 0): float
    {
        ...
        $totalAciert = (float) \App\Models\Result::whereDate('date', $dateStr)->where('user_id', $userId)->sum('aciert');
        
        // Calcular totalApus (excluyendo jugadas anuladas)
        $apusQuery = \App\Models\ApusModel::whereDate('created_at', $dateStr)
            ->where('user_id', $userId)
            ->whereHas('playsSent', function($query) {
                $query->where('status', '!=', 'I');
            });
