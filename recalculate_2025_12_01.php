<?php
/**
 * Script para recalcular resultados del 01-12-2025
 * Usa los extractos que ya están cargados en el sistema
 * 
 * Uso: php recalculate_2025_12_01.php
 * 
 * Este script:
 * 1. Elimina los resultados existentes del 01-12-2025
 ...
 * 4. Incluye apu_id para permitir múltiples resultados del mismo número cuando provienen de diferentes APUs
 */
...
echo "(Incluyendo apu_id para permitir múltiples resultados del mismo número)\n\n";
...
try {
    ...
    // Verificar si hay resultados sin apu_id
    $resultsWithoutApuId = Result::whereDate('date', $date)
        ->whereNull('apu_id')
        ->count();
    
    if ($resultsWithoutApuId > 0) {
        echo "⚠️  Resultados sin apu_id: {$resultsWithoutApuId}\n";
    } else {
        echo "✅ Todos los resultados tienen apu_id asignado\n";
    }
    ...
    $duplicates = \DB::select("
        ...
        FROM (
            SELECT 
                ticket, lottery, number, position, date, apu_id,
                COUNT(*) as cnt
            FROM results 
            WHERE date = ?
    ...
    $duplicateCount = $duplicates[0]->cantidad ?? 0;
    
    if ($duplicateCount > 0) {
        echo "⚠️  Se encontraron {$duplicateCount} grupos de duplicados\n";
    } else {
    ...
    Log::error("recalculate_2025_12_01 - Error: " . $e->getMessage(), [
        'date' => $date,
